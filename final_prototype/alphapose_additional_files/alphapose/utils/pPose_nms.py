...
def write_json(all_results, outputpath, form=None, for_eval=False, outputfile='alphapose-results.json'):
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            #Random Forest
            #Load Model
            RF_Saved_Filename="/content/drive/Shareddrives/Team56_FallDetection/saved_models/RF_model_UR_dept_7"
            clf=pickle.load(open(RF_Saved_Filename,"rb"))

            #Test on Keypoints
            x_test=numpy.asarray(result['keypoints'])
            y_pred=clf.predict((x_test).reshape(1, -1))
            print("RF",y_pred)
            # Append-adds at last
            file1 = open("myfile.txt","a")#append mode
            file1.write("RF "+str(y_pred)+"\n")
            file1.close()
            ...
            new_model = tf.keras.models.load_model("/content/drive/Shareddrives/Team56_FallDetection/saved_models/404_cnn_big_skeleton_alter_2nd_Train_3")
            model=new_model
            xboxtrain=result['box']
            #Load image file path
            file1=open("Holder.txt","r+")
            xtrain=rgb_extract("/content/AlphaPose/examples/res/vis/"+result['image_id'])
            v_min= np.full((1, 1), 0, dtype=int)
            v_max= np.full((1, 1), 640, dtype=int)
            xboxtrain=(xboxtrain - v_min)/(v_max - v_min)
            v_min=np.full((1, 1, 480, 3), 0, dtype=int)
            v_max=np.full((1, 1, 480, 3), 254, dtype=int)
            xtrain=(xtrain - v_min)/(v_max - v_min)
            y_pred_ohe =model.predict([xtrain, xboxtrain])
            y_pred_labels = np.argmax(y_pred_ohe, axis=1)
            print("CNN",y_pred_labels)
            # Append-adds at last
            file1 = open("myfile.txt","a")#append mode
            file1.write("CNN"+str(y_pred_labels)+"\n")
            file1.close()
            ...
            if(y_pred_labels==[0] and y_pred==[0]):
              ...
              # Open an Image
              img = Image.open("/content/AlphaPose/examples/res/vis/"+result['image_id'])
              
              # Call draw Method to add 2D graphics in an image
              I1 = ImageDraw.Draw(img)
              
              # Custom font style and font size
              myFont = ImageFont.truetype('/content/AlphaPose/FreeMono.ttf', 65)
              
              # Add Text to an image
              I1.text((10, 10), "Fall", font=myFont, fill =(255, 0, 0))
              
              # Display edited image
              img.show()
              
              # Save the edited image
              img.save("/content/AlphaPose/examples/res/vis/"+result['image_id'])
            ###############################################################
